import { UserDatabase } from "../database/UserDatabase";
import { NotFoundError } from "../errors/NotFoundError";
import { ConflictError } from "../errors/ConflictError";
import { ParamsError } from "../errors/ParamsError";
import {
  ILoginInputDTO,
  ILoginOutputDTO,
  ISignupInputDTO,
  ISignupOutputDTO,
  User,
  USER_ROLES,
  ILoginOutputDTOadmin,
  IPurchaseInputDTO,
  IPurchaseInputDTO2,
  IPurchaseInputDTO3
} from "../models/User";
import { Authenticator, ITokenPayload } from "../services/Authenticator";
import { HashManager } from "../services/HashManager";
import { IdGenerator } from "../services/IdGenerator";
import { AuthenticationError } from "../errors/AuthenticationError";

export class UserBusiness {
  constructor(
    private userDatabase: UserDatabase,
    private idGenerator: IdGenerator,
    private hashManager: HashManager,
    private authenticator: Authenticator
  ) {}

  public signup = async (input: ISignupInputDTO): Promise<ISignupOutputDTO> => {
    const { name, email, password } = input;

    if (typeof name !== "string") {
      throw new ParamsError("Parâmetro 'name' inválido: deve ser uma string");
    }

    if (typeof email !== "string") {
      throw new ParamsError("Parâmetro 'email' inválido: deve ser uma string");
    }

    if (typeof password !== "string") {
      throw new ParamsError(
        "Parâmetro 'password' inválido: deve ser uma string"
      );
    }

    if (name.length < 3) {
      throw new ParamsError(
        "Parâmetro 'name' inválido: mínimo de 3 caracteres"
      );
    }

    if (password.length < 6) {
      throw new ParamsError(
        "Parâmetro 'password' inválido: mínimo de 6 caracteres"
      );
    }

    if (!email.match(/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/g)) {
      throw new ParamsError("Parâmetro 'email' inválido");
    }

    const isEmailAlreadyExists = await this.userDatabase.findByEmail(email);

    if (isEmailAlreadyExists) {
      throw new ConflictError("Email já cadastrado");
    }

    const id = this.idGenerator.generate();
    const hashedPassword = await this.hashManager.hash(password);
    const user = new User(id, name, email, hashedPassword, USER_ROLES.NORMAL);
    await this.userDatabase.createUser(user);

    const payload: ITokenPayload = {
      id: user.getId(),
      role: user.getRole(),
    };

    const token = this.authenticator.generateToken(payload);

    const userInfo = {
        id: user.getId(),
        name: user.getName()
       }
   
       const response: ISignupOutputDTO = {
         message: "Cadastro realizado com sucesso",
         userInfo,
         token,
       };

    return response;
  };

  public login = async (input: ILoginInputDTO): Promise<ILoginOutputDTO> => {
    const { email, password } = input;

    if (typeof email !== "string") {
      throw new ParamsError("Parâmetro 'email' inválido");
    }

    if (typeof password !== "string") {
      throw new ParamsError("Parâmetro 'password' inválido");
    }

    if (password.length < 6) {
      throw new ParamsError(
        "Parâmetro 'password' inválido: mínimo de 6 caracteres"
      );
    }

    if (!email.match(/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/g)) {
      throw new ParamsError("Parâmetro 'email' inválido");
    }

    const userDB = await this.userDatabase.findByEmail(email);

    if (!userDB) {
      throw new NotFoundError("Email não cadastrado");
    }

    const user = new User(
      userDB.id,
      userDB.name,
      userDB.email,
      userDB.password,
      userDB.role
    );

    const isPasswordCorrect = await this.hashManager.compare(
      password,
      user.getPassword()
    );

    if (!isPasswordCorrect) {
      throw new AuthenticationError("Password incorreto");
    }

    const payload: ITokenPayload = {
      id: user.getId(),
      role: user.getRole(),
    };

    const token = this.authenticator.generateToken(payload);

    const userInfo = {
     id: user.getId(),
     name: user.getName()
    }

    const response: ILoginOutputDTO = {
      message: "Login realizado com sucesso",
      userInfo,
      token,
    };

    return response;
  };
  
  public loginAdmin = async (input: ILoginInputDTO): Promise<string> => {
      const { email, password } = input;
      

    if (typeof email !== "string") {
      throw new ParamsError("Parâmetro 'email' inválido");
    }
    
    if (typeof password !== "string") {
      throw new ParamsError("Parâmetro 'password' inválido");
    }

    if (password.length < 6) {
      throw new ParamsError(
        "Parâmetro 'password' inválido: mínimo de 6 caracteres"
      );
    }
    if (!email.match(/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/g)) {
      throw new ParamsError("Parâmetro 'email' inválido");
    }

    const userDB = await this.userDatabase.findByEmail(email);

    if (!userDB) {
      throw new NotFoundError("Email não cadastrado");
    }

    const user = new User(
      userDB.id,
      userDB.name,
      userDB.email,
      userDB.password,
      userDB.role
    );

    const isPasswordCorrect = await this.hashManager.compare(
      password,
      user.getPassword()
    );

    if (!isPasswordCorrect) {
      throw new AuthenticationError("Password incorreto");
    }

    const payload: ITokenPayload = {
        id: user.getId(),
        role: user.getRole(),
      };
  
      const token = this.authenticator.generateToken(payload);

      const response: any = {
        token,
        role: user.getRole()
      }

    return response;
  };

  public purchase = async (input: IPurchaseInputDTO) => {

    const { id, name, city, road, complement, number, car } = input;
    
    console.log( id, name, city, road, complement, number, car)

  if (typeof name !== "string") {
    throw new ParamsError("Parâmetro 'name' inválido");
  }
  
  if (typeof city !== "string") {
    throw new ParamsError("Parâmetro 'city' inválido");
  }

  if (typeof road !== "string") {
    throw new ParamsError("Parâmetro 'road' inválido");
  }

  if (typeof complement !== "string") {
    throw new ParamsError("Parâmetro 'complement' inválido");
  }
  
  if (typeof number !== "number") {
    throw new ParamsError("Parâmetro 'number' inválido");
  }

  const response = {
    message: ``
  }

 
    if(car.length === 1){ 

      const idTable = this.idGenerator.generate();
      const quantity = car[0].quantity as number
      const product_id =  car[0].id as string

      const inputOne: IPurchaseInputDTO3 = {
        id: idTable,
        user_id : id, 
        name, 
        city, 
        road, 
        complement, 
        number,
        product_id,
        quantity
      }

      response.message = `Produto adicionado com sucesso !`
    const userDB = await this.userDatabase.purchase(inputOne);

    

    return response
      
    }else{
      car.map( async(products: any) =>{

        const idTable = this.idGenerator.generate();
        const quantity = car[0].quantity as number
        const product_id =  car[0].id as string
  
        const inputOne: IPurchaseInputDTO3 = {
          id: idTable,
          user_id : id, 
          name, 
          city, 
          road, 
          complement, 
          number,
          product_id: products.id,
          quantity: products.quantity
        }

        response.message = `Produtos adicionados com sucesso !`
        const userDB = await this.userDatabase.purchase(inputOne);
        


      })
    }

    return response
 
};


}
